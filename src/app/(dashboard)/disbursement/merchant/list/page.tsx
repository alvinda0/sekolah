...
import { Calendar, Eye, Banknote, Store } from "lucide-react";
import { usePageTitle } from "@/hooks/usePageTitle";
import { useRouter } from "next/navigation";
import { withRoleProtection } from "@/components/ProtectedRoles";
import { ErrorState } from "@/components/ErrorState";

const ListDisbursementMerchant = () => {
  usePageTitle("Disbursement Merchant List");
  ...
  const fetchDisbursements = useCallback(async () => {
    try {
      setLoading(true);
      const data = await disbursementMerchantService.getDisbursementMerchants({
        page: 1,
        limit: 100,
      });
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "Failed to fetch disbursement merchants";
  ...
  const handleViewDetail = (disbursementId: string) => {
    router.push(`/disbursement/merchant/detail/${disbursementId}`);
  };
  ...
  const disbursementColumns = [
    {
      name: "Created At",
      selector: (row: DisbursementMerchant) => row.created_at,
      sortable: true,
      width: "150px",
      cell: (row: DisbursementMerchant) => (
    ...
    {
      name: "Merchant Name",
      selector: (row: DisbursementMerchant) => row.merchant_name,
      sortable: true,
      width: "250px",
      cell: (row: DisbursementMerchant) => (
        <div className="flex items-center gap-2">
    ...
    {
      name: "Bank Name",
      selector: (row: DisbursementMerchant) => row.bank_name,
      sortable: true,
      width: "180px",
      cell: (row: DisbursementMerchant) => (
    ...
    {
      name: "Acc. Name",
      selector: (row: DisbursementMerchant) => row.account_name,
      sortable: true,
      width: "150px",
    },
    {
      name: "Acc. Number",
      selector: (row: DisbursementMerchant) => row.account_number,
      sortable: true,
      width: "150px",
    },
    {
      name: "Amount",
      selector: (row: DisbursementMerchant) => row.amount,
      sortable: true,
      width: "150px",
      cell: (row: DisbursementMerchant) => (
        <div className="flex items-center gap-2">
    ...
    {
      name: "Admin Cost",
      selector: (row: DisbursementMerchant) => row.admin_cost,
      sortable: true,
      width: "150px",
      cell: (row: DisbursementMerchant) => (
    ...
    {
      name: "Total",
      selector: (row: DisbursementMerchant) => row.total_disbursements,
      sortable: true,
      width: "150px",
      cell: (row: DisbursementMerchant) => (
    ...
    {
      name: "Type",
      selector: (row: DisbursementMerchant) => row.type,
      sortable: true,
      cell: (row: DisbursementMerchant) => (
        <span
    ...
    {
      name: "Status",
      selector: (row: DisbursementMerchant) => row.status,
      sortable: true,
      width: "150px",
      cell: (row: DisbursementMerchant) => (
        <span
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading disbursement merchants...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <CustomDataTable
        title="Disbursement Merchant Management"
        description="Manage and view all disbursement merchants"
...
export default withRoleProtection(ListDisbursementMerchant, [
  "AgentOwner",
  "AgentStaff",
]);
