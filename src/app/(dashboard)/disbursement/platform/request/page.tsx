...
import { CreateDisbursementPlatformRequest } from "@/types/disbursement-platform";
...
const RequestDisbursementPlatform = () => {
  ...
  const [formData, setFormData] = useState<CreateDisbursementPlatformRequest>({
    platform_id: "",
    amount: 0,
    bank_platform_id: "", // Ganti semua field bank menjadi satu field ini
    type: "IDR",
  });

  const [formErrors, setFormErrors] = useState<
    Partial<Record<keyof CreateDisbursementPlatformRequest, string>>
  >({});

  const fetchBankPlatforms = useCallback(async () => {
    try {
      setBanksLoading(true);
      const data = await bankPlatformService.getBankPlatforms({
        platform_id: user?.platform_id ? parseInt(user.platform_id) : undefined,
        status: "Accepted",
      });
      setBanks(data);
    } catch (err) {
  ...
  const fetchWalletBalance = useCallback(async () => {
    try {
      ...
      const data = await walletService.getWalletPlatforms({
        platform_id: user?.platform_id ? user.platform_id : undefined,
      });
  ...
  useEffect(() => {
    if (user?.platform_id) {
      setFormData((prev) => ({ ...prev, platform_id: user.platform_id }));
      fetchBankPlatforms();
      fetchWalletBalance();
    }
  }, [user?.platform_id, fetchBankPlatforms, fetchWalletBalance]);

  const validateForm = (): boolean => {
    const errors: Partial<
      Record<keyof CreateDisbursementPlatformRequest, string>
    > = {};
    ...
    if (!formData.bank_platform_id || !formData.bank_platform_id.trim()) {
      errors.bank_platform_id = "Bank account is required";
    }
  ...
  const handlePinConfirm = async (pin: string) => {
    try {
      ...
      await disbursementPlatformService.createDisbursementPlatform(payload);
      ...
      setTimeout(() => {
        router.push("/disbursement/platform/list");
      }, 2000);
    } catch (err) {
      const errorMsg =
  ...
  const handleInputChange = (
    field: keyof CreateDisbursementPlatformRequest,
    value: string | number
  ) => {
  ...
  const handleBankSelect = (bankPlatformId: string) => {
    setFormData((prev) => ({
      ...prev,
      bank_platform_id: bankPlatformId, // Cukup simpan ID-nya saja
    }));

    if (formErrors.bank_platform_id) {
      setFormErrors((prev) => ({ ...prev, bank_platform_id: undefined }));
    }
  };
            ...
            <Card className="w-full backdrop-blur-xl bg-white/70 border border-white/20 shadow-2xl">
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription>
                      Submit a new disbursement request for your platform
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
              ...
              <CardContent>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <SelectTrigger
                        className={`backdrop-blur-sm bg-white/50 ${
                          formErrors.bank_platform_id ? "border-red-500" : ""
                        }`}
                      >
                    ...
                    {banks.length === 0 && !banksLoading && (
                      <p className="text-sm text-amber-600">
                        No active bank accounts found. Please add a bank account
                        first.
                      </p>
                    )}
                  </div>
                  ...
                  {/* Selected Bank Details - Tampilkan berdasarkan bank_platform_id */}
                  {formData.bank_platform_id &&
                    (() => {
                      const selectedBank = banks.find(
                        (bank) =>
                          bank.bank_platform_id === formData.bank_platform_id
                      );
                  ...
                  <div className="flex justify-end gap-3 pt-4">
                    ...
                    <Button
                      ...
                      disabled={loading || banksLoading || banks.length === 0}
...
export default withRoleProtection(RequestDisbursementPlatform, [
  "PlatformOwner",
]);
