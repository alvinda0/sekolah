...
const SettlementPlatformDetailPage = () => {
    usePageTitle("Settlement Platform Detail");
    const router = useRouter();
    const params = useParams();
    const settlementId = params?.id as string;

    const [settlement, setSettlement] = useState<SettlementPlatform | null>(null);
    ...
    const fetchSettlementDetail = useCallback(async () => {
        try {
            setLoading(true);
            setError(null);
            const data = await settlementPlatformService.getSettlementPlatformById(settlementId);
            setSettlement(data);
        } catch (err) {
            const errorMessage =
                err instanceof Error
                    ? err.message
                    : "Failed to fetch settlement platform details";
        ...
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-center">
                    ...
                    <p className="text-gray-600">Loading settlement platform details...</p>
                </div>
            </div>
        );
    }
    ...
    if (error || !settlement) {
        return (
            <div className="max-w-7xl mx-auto px-4 py-8">
                ...
                <ErrorState
                    code={getErrorCode()}
                    description={error || "Settlement platform not found"}
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
            ...
            {/* Main Info Card */}
            <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-200 rounded-2xl shadow-lg p-8">
                <div className="flex items-start justify-between mb-6">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 mb-2">
                            Settlement Platform Details
                        </h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white/80 backdrop-blur-sm rounded-xl p-6 border border-white/50 shadow-sm">
                        ...
                        <div className="text-sm text-gray-600">
                            ...
                            <br />
                            <span className="font-mono text-xs">{settlement.platform_id}</span>
                        </div>
                    </div>
...
export default withRoleProtection(SettlementPlatformDetailPage, [
